...
function fieldPhone(e){
	...
	$(e).bind('keyup change', function(event){
	    phoneValid(this, event, true);
			...
			var inputLength = document.querySelector('.inputPhoneNumber').value.length,
					     submit = document.querySelector('.submitData'),
							 color  = inputLength > 9 ? '#D80020' : '#EAEAEA';

			setBkgClr(color);

			function setBkgClr(color) {
				submit.style.backgroundColor = color;
				submit.style.border = '2px solid ' + color;
			}

			if (inputLength > 9) {
				submit.addEventListener('mouseover', mouseover, false);
				submit.addEventListener('mouseout', mouseout, false);

				function mouseover() {
					var inputLength = document.querySelector('.inputPhoneNumber').value.length;
						inputLength > 9 ? submit.style.backgroundColor = '#c0001c' : false;

				};
				function mouseout() {
					var inputLength = document.querySelector('.inputPhoneNumber').value.length;
					inputLength > 9 ? submit.style.backgroundColor = '#D80020' : false;
				};
			}	else {
				submit.removeEventListener('mouseover', mouseover, false);
				submit.removeEventListener('mouseout', mouseout, false);
			};
